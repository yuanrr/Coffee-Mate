    ...
    if sample in ["rand", "middle"]: # uniform sampling
        ...
        if len(frame_indices) < num_frames:  # padded with last frame
            try:
                ...
                padded_frame_indices = [0] * num_frames
...
def read_frames_decord(
    ...
    if clip:
        ...
        if start_index + vlen > total_len:
            start_index = total_len - vlen
...
# TODO add frame dara support
def read_frames_frame(
        video_path, num_frames, sample='rand', fix_start=None,
        max_num_frames=-1, client=None, clip=None
    ):
    vlen = len(os.listdir(video_path))
    fps = 3  # for TVQA and VLEP
    ...
    if clip:
        ...
        try:
            ...
            start, end = 0, int(vlen/fps)

        vlen = min(total_len-1, int(end * fps) - int(start * fps))
        start_index = max(1, int(start * fps))

        if start_index + vlen > total_len:
            start_index = max(1, total_len - vlen)
            vlen = total_len - start_index
    ...
    if clip:
        frame_indices = [f + start_index for f in frame_indices]  # +1 for frame starting from 000001,jpg

    frames = []
    for frame_index in frame_indices:
        img = Image.open(os.path.join(video_path, f"{frame_index:05d}.jpg")).convert('RGB')
        img = PILToTensor()(img)  # C, H, W
        frames.append(img)
    frames = torch.stack(frames)
    return frames, frame_indices, 3  # for TVQA and VLEP
